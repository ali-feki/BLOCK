...
async function loadSession() {
    try {
        ...
        if (credsId.startsWith("STARK-MD:") || credsId.startsWith("STARK-MD≈")) {
            let source, pasteId;
            if (credsId.startsWith("STARK-MD:")) {
                const withoutPrefix = credsId.slice("STARK-MD:".length);
                const colonIdx = withoutPrefix.indexOf(":");
                if (colonIdx === -1) throw new Error("Invalid format — expected STARK-MD:<pb|prs>:<pasteId>");
        ...
        // ── FORMAT 4: BlackHat legacy (BlackHat~<b64>) ───────
        if (credsId.startsWith("BlackHat~") || credsId.startsWith("BlackHat≈")) {
            console.log("🕸️ FORMAT: BlackHat legacy — loading...");
